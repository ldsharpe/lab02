...
class Robot:
    def __init__(self):
        ...
        self.start = [0.5 + 0.09, 0 - 0.09, math.radians(90)]
        self.goal = [2.52, 2.53, 0]
        self.recent_hitpoint = self.start.copy()
        self.distance_to_goal = self.distance_between_points(self.start, self.goal)
        ...
        self.reset_odometry(self.start[0], self.start[1], self.start[2])


        # Precompute M-line coefficients for distance calculation
        x1, y1, _ = self.start
        x2, y2, _ = self.goal
        ...
        self.mline_norm = math.sqrt(self.A**2 + self.B**2)
    ...
    def move_forward(self, distance, speed=350):
        distance += 0.02
    ...
    def move_backward(self, distance, speed=350):
    ...
    def near_goal(self):
        ...
        return (abs(dx) <= 0.03) and (abs(dy) <= 0.03)

    
    def on_m_line(self, tol=0.10):
        d = abs(self.A * self.x + self.B * self.y + self.C) / self.mline_norm
        return d <= tol
